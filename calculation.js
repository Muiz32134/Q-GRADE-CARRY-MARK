...
function displayStudents(students, courseId, calculations) {
    ...
    for (const studentId in students) {
        if (students[studentId].registeredCourses?.[courseId]) {
            ...
            row.innerHTML = `
                <td>${index++}</td>
                <td class="matric-number">${student.stdid || ''}</td>
                <td class="student-name">${student.name || ''}</td>
                <td>${student.code || ''}</td>
                <td><input type="number" name="formative-1-${student.stdid}" value="${previousCalc.formative?.assessment1 || 0}"></td>
                <td><input type="number" name="formative-2-${student.stdid}" value="${previousCalc.formative?.assessment2 || 0}"></td>
                <td><input type="number" name="formative-3-${student.stdid}" value="${previousCalc.formative?.assessment3 || 0}"></td>
                <td><input type="number" name="formative-4-${student.stdid}" value="${previousCalc.formative?.assessment4 || 0}"></td>
                <td><input type="number" name="formative-5-${student.stdid}" value="${previousCalc.formative?.assessment5 || 0}"></td>
                <td><input type="number" name="formative-6-${student.stdid}" value="${previousCalc.formative?.assessment6 || 0}"></td>
                <td class="formative-subtotal">${previousCalc.formativeTotal || 0}</td>
                <td class="formative-percent">${previousCalc.formativePercent || 0}</td>
                <td><input type="number" name="summative-1-${student.stdid}" value="${previousCalc.summative?.assessment1 || 0}"></td>
                <td><input type="number" name="summative-2-${student.stdid}" value="${previousCalc.summative?.assessment2 || 0}"></td>
                <td class="summative-subtotal">${previousCalc.summativeTotal || 0}</td>
                <td class="summative-percent">${previousCalc.summativePercent || 0}</td>
                <td class="total-mark">${previousCalc.totalMark || 0}</td>
                <td class="grade">${previousCalc.grade || '-'}</td>
                <td class="point">${previousCalc.point || 0}</td>
                <td class="status">${previousCalc.status || '-'}</td>
            `;

            tbody.appendChild(row);
            
            // Trigger calculations to update all values
            calculateFormativeSubtotal(row);
            calculateSummativeSubtotal(row);
            calculateTotalMark(row);
            calculateGrade(row);
        }
    }
...
function saveCalculations() {
    ...
    studentRows.forEach(row => {
        ...
        const data = {
            formative: {},
            summative: {},
            formativeTotal: row.querySelector('.formative-subtotal').textContent.trim(),
            formativePercent: row.querySelector('.formative-percent').textContent.trim(),
            summativeTotal: row.querySelector('.summative-subtotal').textContent.trim(),
            summativePercent: row.querySelector('.summative-percent').textContent.trim(),
            totalMark: row.querySelector('.total-mark').textContent.trim(),
            grade: row.querySelector('.grade').textContent.trim(),
            point: row.querySelector('.point').textContent.trim(),
            status: row.querySelector('.status').textContent.trim()
        };

        // Get formative input values
        for (let i = 1; i <= 6; i++) {
            const input = row.querySelector(`input[name="formative-${i}-${studentId}`);
            data.formative[`assessment${i}`] = input ? parseFloat(input.value) || 0 : 0;
        }

        // Get summative input values
        for (let i = 1; i <= 2; i++) {
            const input = row.querySelector(`input[name="summative-${i}-${studentId}`);
            data.summative[`assessment${i}`] = input ? parseFloat(input.value) || 0 : 0;
        }

        calculationData[studentId] = data;
    });

    // Save to Firebase
    return firebase.database().ref(`calculations/${courseId}`).set(calculationData)
        .then(() => {
    ...
    document.querySelectorAll('input[type="number"]').forEach(input => {
        input.addEventListener('input', function() {
            ...
            if (row) {
                ...
                calculateSummativeSubtotal(row);
                calculateTotalMark(row);
                calculateGrade(row); // Add grade calculation
            }
        });
    });
}
